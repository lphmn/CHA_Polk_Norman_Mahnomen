...
filtered_dataTbl <- filtered_data |> 
  ...
  dplyr::rename_with(~ paste0("Child & Teen Check-up ", maxYr-1), valueMinYr)  |> 
  dplyr::rename_with(~ paste0("Child & Teen Check-up ", maxYr), valueMaxYr)
...
Prenatal care is essential for ensuring the health and well-being of both mothers and their babies. Adequate prenatal care helps monitor the progress of the pregnancy, identify and manage potential health issues, and provide important health education to expectant mothers.

In Minnesota, the percentage of mothers receiving late or inadequate prenatal care increased slightly from 8.7% in 2021 to 9.4% in 2022, representing a 0.7% increase. Data for Polk County and Norman County is not available for both years, likely due to suppression rules or the majority of births occurring out of state, possibly in North Dakota. Mahnomen County had a high rate of late or inadequate prenatal care at 29.9% in 2021, but data for 2022 is not available [@kidsCountPreNatalCare]. The Minnesota Department of Health is currently developing a new dashboard that should help address the data suppression limitations identified in the Kids Count data. Please check back in Q2 of 2025 for the most up-to-date information.
...
filtered_dataTbl <- filtered_data |> 
  ...
  dplyr::rename("% Change" = pctDifference) |> 
  dplyr::rename_with(~ paste0("Late/Inadequate PNC ", maxYr-1), valueMinYr)  |> 
  dplyr::rename_with(~ paste0("Late/Inadequate PNC ", maxYr), valueMaxYr)
...
filtered_dataTbl <- filtered_data |> 
  ...
  dplyr::rename("% Change" = pctDifference) |> 
  dplyr::rename_with(~ paste0("Seven-Vaccine Series ", maxYr-1), valueMinYr)  |> 
  dplyr::rename_with(~ paste0("Seven-Vaccine Series ", maxYr), valueMaxYr)
...
filtered_dataTbl <- filtered_data |> 
  ...
  dplyr::distinct() |> 
  dplyr::select(Location, vaccine, valueMinYr, valueMaxYr, pctDifference) |>   
  dplyr::mutate(
    vaccine = vaccine,
    valueMinYr = scales::label_percent(accuracy = .01)(valueMinYr)
    , valueMaxYr = scales::label_percent(accuracy = .01)(valueMaxYr)
  ...
  dplyr::rename("% Change" = pctDifference, "Vaccine" = vaccine) |>    
  dplyr::rename_with(~ as.character(maxYr-1), valueMinYr)   |>  
  dplyr::rename_with(~ as.character(maxYr), valueMaxYr) #rename_with requires character vactor not a number  

#split table into one group for pdf output. This is done here because I can design the columns for when the report is rendered eaiser than trying to loop through the data and create different div/Quarto layouts
filtered_dataTblMn <- filtered_dataTbl |> 
    dplyr::filter(Location %in% c("Minnesota"))  
filtered_dataTblMa <- filtered_dataTbl |> 
    dplyr::filter(Location %in% c("Mahnomen"))
filtered_dataTblNo <- filtered_dataTbl |> 
    dplyr::filter(Location %in% c("Norman"))  
filtered_dataTblPo <- filtered_dataTbl |> 
    dplyr::filter(Location %in% c("Polk"))
...
scaleYContinuous = scales::label_percent()  # Format y-axis values with geography
#Table
colList <- colnames(filtered_dataTbl) # list("Year", "Location", "Vaccine", "Percentage", "% Change") # Using a list accommodates tables with different column lengths.
...
gbFun_tablePDF(filtered_dataTblMn, colList)
}
```
...
if (!knitr::is_html_output()) {
#Tables shouldn't be printed because the HTML table will display
gbFun_tablePDF(filtered_dataTblMa, colList)
}
```
:::
...
gbFun_tablePDF(filtered_dataTblNo, colList)
}
```
...
if (!knitr::is_html_output()) {
#Tables shouldn't be printed because the HTML table will display
gbFun_tablePDF(filtered_dataTblPo, colList)
}
```
:::
:::
