...
filtered_dataTbl <- filtered_data |> 
  ...
  dplyr::summarize(
    Location = location
    , valueMinYr = percentage[year == min(year)]
    , valueMaxYr = percentage[year == max(year)]
  ...
  dplyr::rename_with(~ paste0("Reading Proficiency ", maxYr-1), valueMinYr)  |> 
  dplyr::rename_with(~ paste0("Reading Proficiency ", maxYr), valueMaxYr)

#####Long format no longer wanted ####################
# filtered_dataTbl <- filtered_data |> 
#   dplyr::filter(year >= maxYr-1)  |>
#   dplyr::arrange(fips, year) |>
...
filtered_dataTbl <- filtered_data |> 
  ...
  dplyr::rename("% Change" = pctDifference) |> 
  dplyr::rename_with(~ paste0("Math Proficiency ", maxYr-1), valueMinYr)  |> 
  dplyr::rename_with(~ paste0("Math Proficiency ", maxYr), valueMaxYr)
...
#   dplyr::group_by(fips) |> 
#   dplyr::mutate(percentageChange = scales::label_percent(accuracy = .01)((rate) - dplyr::first(rate)) #Since there is only 2, most recent - next
#   ) |>
#   dplyr::mutate(percentageChange = dplyr::case_when(
...
#    ) |>
#   dplyr::ungroup() |>
#   dplyr::select(year, location, placeholder, percentageChange)      
...
filtered_dataTbl <- filtered_data |> 
  ...
  dplyr::summarize(
    Location = location
    , rateMinYr = rate[year == min(year)]
    , rateMaxYr = rate[year == max(year)]
    , pctDifference = rateMaxYr - rateMinYr
    , .groups = 'drop'
  ) |>
  dplyr::distinct() |> 
  dplyr::select(-fips) |> 
  dplyr::mutate(
    rateMinYr = scales::label_percent(accuracy = .01)(rateMinYr)
    , rateMaxYr = scales::label_percent(accuracy = .01)(rateMaxYr)
    , pctDifference = scales::label_percent(accuracy = .01)(pctDifference)
    ) |>
  dplyr::rename("% Change" = pctDifference) |> 
  dplyr::rename_with(~ paste0("Unemployment Rate ", maxYr-1), rateMinYr)  |> 
  dplyr::rename_with(~ paste0("Unemployment Rate ", maxYr), rateMaxYr)
...
p <- gbFun_plotTrendPDF(
    ...
    gbVal_shapeScheme, ifelse(knitr::is_html_output(), 10, 12)
    )
...
filtered_dataTbl <- filtered_data |> 
  ...
  dplyr::summarize(
    Location = location
    , rateMinYr = dentalServicePercent[year == min(year)]
    , rateMaxYr = dentalServicePercent[year == max(year)]
    , pctDifference = rateMaxYr - rateMinYr
    , .groups = 'drop'
  ) |>
  dplyr::distinct() |> 
  ...
  dplyr::rename("% Change" = pctDifference) |> 
  dplyr::rename_with(~ paste0("Medicaid Beneficiaries ", maxYr-1), rateMinYr)  |> 
  dplyr::rename_with(~ paste0("Medicaid Beneficiaries ", maxYr), rateMaxYr)
...
#Define the plot here so it is only wrote once
p <- gbFun_plotTrendPDF(
    filtered_data, "year", "dentalServicePercent", "location", "fips", "toolTipTextHeader1",
...
Adverse Childhood Experiences, or ACEs, are preventable, potentially traumatic events that occur in childhood (0-17 years) such as neglect, experiencing or witnessing violence, and having a family member attempt or die by suicide. Also included are aspects of a child's environment that can undermine their sense of safety, stability, and bonding, such as growing up in a household with substance use; mental health problems; or instability due to parental separation or incarceration of a parent, sibling or other member of the household [@cdcAces]. Adverse childhood experiences can increase a person's risk for chronic stress and adverse coping mechanisms and result in lifelong chronic illness such as depression, heart disease, obesity and substance abuse.
